...
const DeleteModal = ({
  ...
  const handleDeleteButton = async () => {
    ...
    const response = await networkManager
      .post(getDevUrl(deleteTalentEndpoint), {
        actorId: actor._id,
      })
      .catch((err) => {
        console.log(err);
    ...
    if (response.success) {
      ...
      networkManager.get(getDevUrl(getTalentsEndpoint)).then((response) => {
        setActors(response.data.data);
      });
    } else {
      toast.error("An error occurred while deleting actor.");
    }
  };
  ...
  return (
    ...
    >
      <div className="modal-box p-2 max-w-2xl relative">
        <h3 className="font-bold text-lg p-2">Delete Actor</h3>
        <hr />
