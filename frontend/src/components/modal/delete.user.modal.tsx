...
const DeleteModal = ({
  ...
  const handleDeleteButton = async () => {
    ...
    const response = await networkManager.post(getDevUrl(deleteUserEndpoint), {
      userId: user._id,
    });
    ...
    if (response.success) {
      ...
      networkManager.get(getDevUrl(getUsersEndpoint)).then((response) => {
  ...
  return (
    ...
    >
      <div className="modal-box p-2 max-w-2xl relative">
        <h3 className="font-bold text-lg p-2">Delete User</h3>
        <hr />
